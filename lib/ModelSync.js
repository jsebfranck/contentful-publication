...
ModelSync.prototype.run = function() {
  ...
  return client.getSpace(self.config.destinationSpace)
    .catch(function(error) {
    ...
    return client.getSpace(self.config.sourceSpace);
  }).catch(function(error) {
    logger.error('Could not find Space %s using access token %s', self.config.sourceSpace, self.config.sourceAccessToken);
    return error;
  }).then(function(space) {
    spaceSource = space;
