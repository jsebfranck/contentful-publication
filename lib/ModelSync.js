...
ModelSync.prototype.run = function() {
  ...
  return client.getSpace(self.config.spaceDestination)
    .catch(function(error) {
    logger.error('Could not find Space %s using access token %s', self.config.spaceDestination, self.config.accessTokenDestination);
    return error;
  }).then(function(space) {
    spaceDestination = space;
    return client.getSpace(self.config.spaceSource);
  }).catch(function(error) {
    logger.error('Could not find Space %s using access token %s', self.config.spaceSource, self.config.accessTokenDestination);
    return error;
  }).then(function(space) {
    spaceSource = space;
