...
ContentSync.prototype.run = function() {
  ...
  var contentfulClient = contentful.createClient({
    accessToken: self.syncClient.config.destinationAccessToken
  });

  return contentfulClient.getSpace(self.syncClient.config.destinationSpace).catch(function(error) {
    logger.error('Could not find Space %s using access token %s', self.syncClient.config.destinationSpace, self.syncClient.config.destinationAccessToken);
    return error;
  }).then(function(space) {
