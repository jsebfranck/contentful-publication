...
Item.prototype.processEntryUpdate = function(space) {
  ...
  return space.getEntry(entryId).then(function(entry) {
    ...
    logger.error('Cannot publish entry %s : %j', entryId, error);
  });
};
...
Item.prototype.processUpdate = function() {
  ...
  if (self.data.sys.type == 'Asset') {
    ...
    return self.processEntryUpdate(self.destinationSpace).then(function(obj) {
      resolver.fulfill(obj);
    });
  }

  logger.info('update ' + self.data.sys.type + ' : ', self.data.sys.id);
  return promise;
};
